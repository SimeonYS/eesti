...
class EestiPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('eesti.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `eesti`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM eesti WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO eesti (date, title, link, content)"
